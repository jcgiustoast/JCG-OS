...
- **Significance:** clears ASTEROI's single biggest bottleneck and retires the #1 strategic risk. In milestone terms (the $10M plan drafted this session), this is **Track A / A1 complete**.
- **Constraint reframe:** the binding constraint moves from **capacity → demand.** Next gates are **A2** (Tincho delivers a full client cycle solo, zero Juan touches) and **Track B** (a repeatable acquisition engine — pipeline is currently empty: VAHDAM dead, Jello declined, no content-sourced inbound yet).
- **Manly delivery upgrade:** with Tincho free from Aug, full Tincho-operated CRO can ramp **during** the initial term (Aug–Oct), not just at renewal — earlier "LP-led ramp only" constraint lifted.
- Pages updated: life/wiki/projects.md (Tincho situation, description, Manly delivery, changelog), life/wiki/strategy.md (Tincho Resolution + risk row → resolved), life/wiki/asteroi-rate-card.md (delivery-model dependency → resolved).
...
- **Manly economics (from their LP dashboard):** ~A$670K/mo total spend across LPs, best page ~A$200K/mo, ROAS 1.16–1.74 (CM breakeven ≈1.43 at 30% COGS → roughly half their spend is underwater). At a full A$200K/mo run-rate, the 30-day window captures ~A$200K → fee ~A$2K per winning page one-time (~$1.3K USD); ~A$2K/mo at ~1 winner/month (~A$6K over the 3-month term). A modest, clean launch bonus — not the uncapped upside originally floated. (Window cut 60→30 on 2026-06-18.) (Confirms Manly > VAHDAM on spend.)
- **Principle 8 generalized:** from "perf fees = Paid Media only" to two spend-keyed patterns — Paid Media → % of CM (VAHDAM); LP-launch → % of new-page spend, 30-day window (Manly). CRO experimentation stays flat (no clean spend lever).
