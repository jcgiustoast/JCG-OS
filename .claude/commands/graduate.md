...
2. Find paragraphs, bullets, or asides that:
   ...
   - Aren't already captured in a wiki page (check indexes)
...
1. Create each approved file with full frontmatter (per `CLAUDE.md` convention — `title`, `description`, `type`, `sources`, `related`, `created: today`, `updated: today`, `confidence: low` unless otherwise obvious).
2. Update the relevant `wiki/index.md` to add the new entry.
3. Append an entry to the relevant `memory/log.md`:
   ```
   ## [YYYY-MM-DD] update | Graduated N ideas into pages
   - Pages created: wiki/foo.md, wiki/bar.md
   ```
4. Ask Juan if he wants to commit.
